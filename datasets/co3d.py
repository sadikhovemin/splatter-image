...
print("HELLO WORLD! BURDAYIM")

CO3D_DATASET_ROOT = "/storage/group/dataset_mirrors/01_incoming/CO3D"  # Change this to where you saved preprocessed data
print("BIR DE BURDAYIM")
...
class CO3DDataset(SharedDataset):
    def __init__(self, cfg, dataset_name="train"):
        ...
        print("CO3D_DATASET_ROOT : ", CO3D_DATASET_ROOT)
        ...
        exclude_sequences = (
            NO_FG_COND_FRAME_SEQ[cfg.data.category[:-1]]
        ...
        for frame_order_file in frame_order_files:
            if (
                os.path.basename(os.path.dirname(frame_order_file))
                not in exclude_sequences
            ):
                if (
                    os.path.basename(os.path.dirname(frame_order_file))
                    not in self.Ts.keys()
    ...
    def get_origin_distance(self, camera_to_world):
        ...
        origin_distances = torch.dot(
            camera_center_to_origin, camera_z_vector
        ).unsqueeze(0)
        origin_distances = repeat(
            origin_distances,
            "c -> c h w",
            h=self.cfg.data.training_resolution,
            w=self.cfg.data.training_resolution,
        )
    ...
    def load_example_id(
        self, example_id, intrin_path, trans=np.array([0.0, 0.0, 0.0]), scale=1.0
    ):
        """
        ...
        focals_folder_path = os.path.join(self.base_path, os.path.basename(dir_path))
        ...
        if example_id not in self.all_rgbs.keys():
            ...
            for cam_info in cam_infos:
                ...
                self.all_focals_pixels[example_id].append(
                    torch.tensor(
                        [fov2focal(cam_info.FovX, 128), fov2focal(cam_info.FovY, 128)]
                    )
                )

                ray_dirs = self.ray_dirs.clone()[0]
                ray_dirs[:2, ...] = ray_dirs[:2, ...] / self.all_focals_pixels[
                    example_id
                ][-1].unsqueeze(1).unsqueeze(2)
                self.all_ray_embeddings[example_id].append(ray_dirs)

                self.all_origin_distances[example_id].append(
                    self.get_origin_distance(
                        self.all_view_to_world_transforms[example_id][-1]
                    )
                )
            ...
            self.all_focals_pixels[example_id] = torch.stack(
                self.all_focals_pixels[example_id]
            )
            self.all_origin_distances[example_id] = torch.stack(
                self.all_origin_distances[example_id]
            )
            self.all_ray_embeddings[example_id] = torch.stack(
                self.all_ray_embeddings[example_id]
            )
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name == "train":
            ...
            frame_idxs = torch.cat(
                [
                    torch.tensor(input_idxs),
                    torch.tensor(
                        [
                            i
                            for i in range(len(self.all_rgbs[example_id]))
                            if i not in input_idxs
                        ]
                    ),
                ],
                dim=0,
