...
def main(cfg: DictConfig):
    ...
    if cfg.data.category in ["nmr", "objaverse", "front3d"]:
        num_workers = 12
    ...
    print("epocs to run", (cfg.opt.iterations + 1 - first_iter) // len(dataloader) + 1)
    ...
    for num_epoch in range(
        ...
        for data in dataloader:
            ...
            if (iteration + 1) % cfg.logging.val_log == 0 and fabric.is_global_zero:
                ...
                if cfg.opt.ema.use:
                    scores = evaluate_dataset(
                        ema, val_dataloader, device=device, model_cfg=cfg
                    )
                else:
                    scores = evaluate_dataset(
                        gaussian_predictor, val_dataloader, device=device, model_cfg=cfg
                    )
            ...
            print("fnames_to_save size", len(fnames_to_save))
            ...
            for fname_to_save in fnames_to_save:
                ...
                print("SAVING THE MODEL TO ", os.path.join(vis_dir, fname_to_save))
